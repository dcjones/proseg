...
pub fn estimate_transcript_density(
        transcripts: &Vec<Transcript>, ngenes: usize, layer_depth: f32, σ: f32,
        binsize: f32, k: usize, eps: f32) -> (Array1<f32>, Array1<f32>) {
    ...
    for gene in 0..ngenes {
        ...
        // Gaussian blur
        let density = density.conv_2d_fft(
            &kernel,
            PaddingSize::Same,
            PaddingMode::Zeros,
...
fn gaus_kernel(binsize: f32, σ: f32, k: usize) -> Array2<f32> {
    ...
    for i in 0..(1 + 2*k) {
        for j in 0..(1 + 2*k) {
            kernel[[i, j]] =
                normal_pdf(σ, (i as f32 - k as f32) * binsize) *
